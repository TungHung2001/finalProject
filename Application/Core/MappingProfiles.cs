...
namespace Application.Core
{
    public class MappingProfiles : AutoMapper.Profile
    {
        public MappingProfiles()
        {
            ...
            CreateMap<Posts, PostFullDTO>()
                .ForMember(i => i.Categories, options => options.Ignore())
                .ForMember(i => i.CategoryIds, options => options.Ignore());
            CreateMap<Posts, PostShortDTO>();
            CreateMap<PostCreateDTO, Posts>()
                //.ForMember(x => x.image, options => options.Ignore())
                //.ForMember(x => x.Image1, options => options.Ignore())
                //.ForMember(x => x.Image2, options => options.Ignore())
                //.ForMember(x => x.Image3, options => options.Ignore())
                //.ForMember(x => x.Image4, options => options.Ignore())
                ;
